...
class AdjItem extends Model
{
    ...
    protected static function boot()
    {
        ...
        static::updated(function ($model) {
            ...
            $dataInduk = AdjItem::find($model->id);
            $dataInduk->update(['updated_by' => auth()->user()->id]);
            OrderItem::where('adj_item_id', $model->id)->update(['updated_by' => auth()->user()->id, 'status' => $model->status, 'date_order' => $model->date_order]);
            ...
            $sum_weight = OrderItem::where('adj_item_id', $model->id)->sum('total_weight');
            AdjItem::where('id', $model->id)->update(['total_weight' => $sum_weight]);

            /// update jurnal STOK OPNAME
            if ($dataInduk->grand_total >= 0) {
                $mutation_type = 'Barang Stok Bertambah';
                $debit = 'NR-DB-B-2000 Persediaan Barang Dagang';
                $kredit = 'LR-KR-E-9300 Barang Stok Bertambah';
                $nominal_plus = $dataInduk->grand_total;
                $nominal_mins = 0;
                $nominal = $dataInduk->grand_total;
            } else {
                $mutation_type = 'Barang Stok Berkurang';
                $debit = 'LR-DB-F-1300 Barang Stok Berkurang';
                $kredit = 'NR-DB-B-2000 Persediaan Barang Dagang';
                $nominal_plus = 0;
                $nominal_mins = abs($dataInduk->grand_total);
                $nominal = abs($dataInduk->grand_total);
            }
            Payment::where('paymentable_type', AdjItem::class)

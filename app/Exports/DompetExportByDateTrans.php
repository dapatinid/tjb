...
class DompetExportByDateTrans implements FromCollection, WithMapping, WithHeadings
{
    ...
    {
        if ($this->bydate != null) {
            return Payment::where('branch_id', Auth::user()->branch_id)->whereNotNull('rekening')->where('rekening', 'LIKE', '%' . $this->rek . '%')->orderBy('date_payment', 'desc')->whereBetween('date_payment', [$this->bydate . ' 00:00:00', $this->bydate . ' 23:59:59'])->get();
        } else {
            return Payment::where('branch_id', Auth::user()->branch_id)->whereNotNull('rekening')->where('rekening', 'LIKE', '%' . $this->rek . '%')->orderBy('date_payment', 'desc')->get();
        }
    }
}

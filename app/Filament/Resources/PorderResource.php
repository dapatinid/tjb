                ...
                Group::make()->schema([
                        ...
                        Repeater::make('items')
                            ...
                            ->schema([
                                ...
                                Select::make('product_id')
                                    ...
                                    ->getOptionLabelFromRecordUsing(fn(Model $record) => "{$record->name} {$record->variant} Rp" . Number::format($record?->cogs ?? 0, locale: 'de'))
                    ...
                    Section::make('Payments')->schema([
                        Repeater::make('payments')
                            ->relationship(
                                name: 'payments',
                                modifyQueryUsing: fn(Builder $query) => $query->where('mutation_type', 'Purchase'),
                            )
                            // ->reorderable()
                            ->deleteAction(
                                fn(Action $action) => $action->hidden(fn() => Auth::user()->level !== "backofficer"),

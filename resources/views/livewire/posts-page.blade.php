<div>
   <section class="max-w-3xl md:p-3 mx-auto dark:text-gray-400 text-gray-500">
    ...
    <div class="block my-3 space-y-3">
      @php
          if (Auth::check()) {
            $postQuery = $posts->where('branch_id', auth()->user()->branch_id);
          } else {
            $postQuery = $posts;
          }
      @endphp
        ...
        <div wire:key="{{ $post->id }}" class="group grid bg-white shadow-sm md:rounded-md hover:shadow-md transition dark:bg-slate-900 dark:border-gray-800 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
          <div class="p-4 md:p-5">
            <div class="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 mb-3 pb-3">
              <div class="flex items-center">
                <img class="md:size-14 size-10 rounded-full object-cover" src="{{ !empty($user->where('id', $post->created_by)->value('image')) ? Str::replace('%2F', '/',url('storage', $user->where('id', $post->created_by)->value('image'))) : url('storage/users/avatar/user.png') }}" alt="{{ $post->title }}">
                <div class="md:ms-4 ms-2">
                  <h3 class="line-clamp-1 text-md font-semibold text-gray-700 md:text-2xl dark:text-gray-200">
                    {{ $post->title }}
                  </h3>
                  <div class="text-xs">{{ $user->where('id', $post->created_by)->value('name') }}</div>
                </div>
              </div>
              <div class="ps-3 text-xs text-end text-nowrap">
                {{ Str::take($post->date_published,10) }} <br>
                {{ Str::substr($post->date_published,10,6) }}
              </div>
            </div>

            <a wire:navigate href="/posts/{{ $post->slug }}">
                <div class="[&>ul]:list-disc [&>ul]:ml-5 mb-3">
                    <p>
                        {{ $post->subtitle }}
                    </p>
                </div>
            ...
            @if (isset($post->cover) )                
            <a wire:navigate href="/posts/{{ $post->slug }}">
                <img class="w-full aspect-square object-cover" src="{{ isset($post->cover) ? Str::replace('%2F', '/',url('storage', $post->cover)) : '' }}" alt="{{ $post->title }}">
            </a>
            @endif
            ...
            $likes = ($post->likes) ? count($likes).' Suka' : '' ;
            @endphp
            <div class="mt-3 flex justify-between">
              <div class="flex {{ $likes ? 'gap-4' : ''}}">
                <span>{{ $likes }}</span>
                <span>{{ $post->comments->count() != 0 ? $post->comments->count().' Komentar' : '' }} </span>
              </div>
                <a wire:navigate href="/posts/{{ $post->slug }}" class="text-blue-400">Selengkapnya...</a>
              </div>
            <div class="grid grid-cols-3 items-center border-t border-gray-200 dark:border-gray-700 mt-3 pt-2 -mb-2">
              ...
              <a class="cursor-pointer flex flex-nowrap items-center px-3 py-2 mx-auto hover:bg-gray-200 dark:hover:bg-white rounded-lg" wire:navigate href="/posts/{{ $post->slug }}#comment">
                <x-far-comment class="md:size-6 size-4"/><span class="md:text-md text-xs ms-2">Komentar</span>
              </a>  
              <a class="cursor-pointer flex flex-nowrap items-center px-3 py-2 mx-auto hover:bg-gray-200 dark:hover:bg-white rounded-lg" href="https://web.whatsapp.com/send?text={{ URL::to('/').'/posts/'.$post->slug  }}"  target="_blank">

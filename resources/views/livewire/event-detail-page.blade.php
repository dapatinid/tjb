<div class="w-full max-w-5xl py-0 mb-3 md:px-10 md:py-10 mx-auto dark:text-gray-100">
    ...
    <a href="{{ Str::replace('%2F', '/',url('storage', $event->cover)) }}" target="_blank">
        <img class="w-full aspect-video object-cover" src="{{ isset($event->cover) ? Str::replace('%2F', '/',url('storage', $event->cover)) : '' }}" alt="{{ $event->title }}">
    </a>
    @endif
    ...
    <div class="overflow-hidden bg-white font-poppins dark:bg-gray-800">
        ...
        <div class="w-full sm:px-10 px-3 text-center border-b-5 border-gray-100 dark:border-slate-700">
            ...
            <a class="cursor-pointer flex flex-nowrap items-center px-3 py-2 mx-auto hover:bg-gray-200 dark:hover:bg-white rounded-lg" href="https://web.whatsapp.com/send?text={{ URL::to('/').'/donates/'.$event->slug  }}"  target="_blank">
            ...
            <a class="cursor-pointer flex flex-nowrap items-center px-3 py-2 mx-auto hover:bg-gray-200 dark:hover:bg-white rounded-lg"  href="#comment">
                <x-far-comment class="md:size-6 size-4"/><span class="md:text-md text-xs ms-2">{{ $event->comments->count() != 0 ? $event->comments->count() : '' }}</span>
            </a>  
            </div>
        </div>  
        ...
        @if ($event->embed_videos)
        @php
            $videos = Str::of($event->embed_videos)->explode(',');
        @endphp
        ...
        <div class="w-full text-center border-t-5 border-b-5 border-gray-100 dark:border-slate-700 p-5">
            ...
            @endif
            <div>
                di tulis pada {{ $event->created_at }} di edit pada {{ $event->updated_at }}
            </div>
        </div>   

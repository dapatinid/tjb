<div class="w-full max-w-[85rem] pt-5 px-4 sm:px-4 lg:px-4 mx-auto pb-16">
  ...
  <div class="container mx-auto">
    ...
    @forelse ($grup_items as $gr_item )
        ...
        <a wire:navigate href="/checkout?branch_id={{ $gr_item['branch_id'] }}&shipping_method=self_pickup&sales_type=self_pickup&payment_method=cash&rekening=KAS+KASIR&date_order={{ date('Y') }}-{{ date('m') }}-{{ date('d') }}T{{ date('H') }}%3A{{ date('i') }}"><button class="w-full px-4 py-2 mt-5 text-center text-white bg-blue-500 rounded-lg hover:bg-green-500">Checkout @currency($cart_items->where('branch_id', $gr_item['branch_id'])->sum('total_amount'))</button></a>
        </div>
